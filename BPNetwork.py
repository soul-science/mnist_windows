...
class BPNetwork(object):
    ...
    """
    def __init__(self, queue, learningSpeed=0.001, penaltyCoefficient=0.001, activation="relu"):
        self.__activations = {"relu": [self._relu, self._relu_backward],
                              "sigmoid": [self._sigmoid, self._sigmoid_backward],
                              "tanh": [self._tanh, self._tanh_backward]}
    ...
    def _sigmoid(self, h):
        """
        # TODO: the active coating which name is called Sigmod
        :param h:
            H = XW + b, H
        :return:
            1/(1 + exp(-h)) => (0, 1)
    ...
    def _sigmoid_backward(self, dh, h):
        """
        # TODO: the affine backward of Sigmod
        """
        return dh * self._sigmoid(h)*(1 + self._sigmoid(h))
    ...
    def _tanh_backward(self, dh, h):
        """
        # TODO: the affine backward of tanh
        """
        return dh * (1 - np.power(self._tanh(h), 2))
    ...
    def fit(self, train_x, train_y, repeat=1000):
        ...
        for i in range(1, repeat + 1):
            ...
            self.queue.put((i, loss, self.score(train_x, train_y)))
